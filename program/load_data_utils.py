...
def get_channels_from_files(input_folder):
    #plate name
...
def get_names_of_channels_from_user(channel_inputs):
    list_channels = [s.replace(" ", "") for s in channel_inputs]
    names_inputs = eg.multenterbox("Write a name to represent each channel we found (ex: DAPI is DNA):", "Names", channel_inputs)
    list_names = [s.replace(" ", "") for s in names_inputs]
    ch_dic = dict(zip(list_channels, list_names))
    for key, value in ch_dic.items():
        print(f"\n Channel: {key}, Name: {value}")
...
#     dirlist = eg.enterbox("Enter the NUMBER of channels in this folder:")
...
#     names_inputs = eg.multenterbox("Write a name to represent each channel (ex: DAPI is DNA):", "Names", channel_inputs)
#     list_channels = [s.replace(" ", "") for s in channel_inputs]
#     list_names = [s.replace(" ", "") for s in names_inputs]
#     ch_dic = dict(zip(list_channels, list_names))
#     for key, value in ch_dic.items():
